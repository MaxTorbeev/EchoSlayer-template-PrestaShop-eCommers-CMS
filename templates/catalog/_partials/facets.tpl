    ...
    {foreach from=$facets item="facet"}
        {if $facet.displayed}
            <section class="facet">
                ...
                {/foreach}
                <div class="title hidden-md-up" data-target="#facet_{$_expand_id}"
                     data-toggle="collapse"{if !$_collapse} aria-expanded="true"{/if}>
                            ...
                            {if $filter.displayed}
                                <li>
                                    {*<label class="custom-control custom-checkbox">*}
                                        {*<input data-search-url="{$filter.nextEncodedFacetsURL}" class="custom-control-input" type="checkbox" {if $filter.active } checked {/if} >*}
                                    ...
                                    <label class="facet-label{if $filter.active} active {/if}">
                                        ...
                                        {else}
                                            <span class="custom-checkbox">
                                                <input data-search-url="{$filter.nextEncodedFacetsURL}" type="radio" name="filter {$facet.label}" {if $filter.active } checked {/if} >
                                                <span {if !$js_enabled} class="ps-shown-by-js" {/if}><i class="material-icons checkbox-checked">&#xE5CA;</i></span>
                                            </span>
                                        {/if}
